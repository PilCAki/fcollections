...
def random_tests():
    ...
    print(f"first: {a.first}, second: {a.second}, last: {a.last}")
    print(a.diff(b))
    ...
    import numpy as np
    print(f'pipe: {a.pipe(lambda x:x*3, np.asarray, np.std)}')
    print(f'pipe_map: {a.pipe_map(lambda x:x-4, lambda x:x+4, lambda x:x*2, lambda x:x-3)}')
